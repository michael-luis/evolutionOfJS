$(document).ready(function()
{
    ...
    {
        $("<div/>", { 'class': 'message', text: message }).hide().prependTo("body").slideDown('fast').delay(3000).slideUp(function() { $(this).remove(); });
    };
    ...
    var render = function()
    {
        ...
        var template = '<div class="thumbnail"><img src="{src}" alt="{name}"/></div>';
        $.getJSON("data/upload.json", function(response)
        {
            ...
            if (album !== '' && album !== null)
            {
                ...
                { // excluding default group, map to format {id: xxx, name: xxx} as source
            ...
            {
                picturesHtml += templateEngine(template, {"src": "uploads/" + element.image, "name": element.name});
            });
